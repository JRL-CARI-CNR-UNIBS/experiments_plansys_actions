...
for (start, action_instance, duration) in plan.timed_actions:
  ...
  time_vars[action] = model.NumVar(0, 1000, f"time_{action}")
  variances[action] = 2
  nominal_value[action] = duration
  print(duration)
# for action in plan.timed_actions:
#     action_str = f'{action[1].action.name}{action[1].actual_parameters}'
#     time_vars[action_str] = model.NumVar(0, 1000, f"time_{node}")
#     variances[action_str] = 2
#     nominal[action_str] = action[0]
