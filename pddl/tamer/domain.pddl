(define (domain pddl-domain)
  ...
  (:predicates 
    ...
    (waypoint_of_room ?w - waypoint ?r_0 - room)
    (cleaning_waypoint ?w - waypoint)
    (patrolling_waypoint ?w - waypoint)
  )
  (:functions 
    (duration_move ?r - robot ?from_w - waypoint ?to_w - waypoint)
    (duration_clean ?r - robot ?rm - room ?w - waypoint)
    (duration_patrol ?r - robot ?w - waypoint)
    (duration_attend_person ?r - robot ?w_welcom - waypoint ?w_dest - waypoint)
  )
  (:durative-action move
    ...
    :effect (and 
              (at start (not (robot_at ?r ?from_w)))
              (at end (robot_at ?r ?to_w))
            )
  )
